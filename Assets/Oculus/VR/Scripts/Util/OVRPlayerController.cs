...
public class OVRPlayerController : MonoBehaviour
{
    ...
    public float SprintAcceleration = 0.12f;
    ...
    void Update()
    {
        ...
        // Footstep sounds
        if (transform.position != prevPos) footstepsSound.enabled = true;
    ...
    public virtual void UpdateMovement()
    {
        ...
        if (EnableLinearMovement)
        {
            ...
            bool moveSprint = Input.GetKey(KeyCode.LeftShift);
            ...
            if (OVRInput.Get(OVRInput.RawButton.A) && (OVRInput.Get(OVRInput.Axis2D.PrimaryThumbstick).y > 0.0f))
            {
                moveSprint = true;
                if (CurrentStamina > 0f) CurrentStamina -= 0.1f;
                else moveSprint = false;
            }
            else if (!(CurrentStamina >= 100f)) CurrentStamina += 0.1f;

            // Gère la stamina bar
            staminaBar.value = CurrentStamina;
            if (CurrentStamina == MaxStamina) staminaBar.gameObject.SetActive(false);
            ...
            if (moveSprint)
            {
                footstepsSound.pitch = 2f;
                moveInfluence = SprintAcceleration * 0.1f * MoveScale * MoveScaleMultiplier;
            }
            else footstepsSound.pitch = 1f;
